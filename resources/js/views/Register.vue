  ...
  <div id="loginForm" class="pt-5">
    <b-form v-on:submit.prevent="signUp" class="mx-auto justify-content-center w-50">
      <b-form-group id="registerErrors" class="errorsBlock" v-show="containsErrors()">
        <ul>
          <li v-for="error in this.$store.getters.getRegisterErrors">{{ error.message }}</li>
        </ul>
      </b-form-group>

      <b-form-group horizontal label="Full Name: " label-for="fullName">
        <b-form-input
          id="fullName"
          name="fullName"
          type="text"
      ...
      <b-form-group horizontal label="Username: " label-for="login">
        <b-form-input
          id="login"
          name="login"
          type="text"
